	...
	*/
	public function get_xml()
	{
		...
		if($this->validateProductData($p)) {
			...
			$params['description'] = substr(get_post($item[$p->id])->post_excerpt,0,5000);
			...
			$params['category'] = get_option('tfm_shrike_setting_category','');
	...
	private function validateProductData()
	{
		...
		if(
			...
			strlen(get_post($item[$p->id])->post_excerpt)==0 ||
